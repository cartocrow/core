...
template <class VertexData, class EdgeData, GraphCurveTraits_2 CurveTraits> class Graph_2 {
	...
	friend class Graph_vertex_map_base;
  ...
  private:
	...
	std::vector<Graph_vertex_map_base*> m_vertex_maps;

	void addVertexMap(Graph_vertex_map_base* map) {		
		m_vertex_maps.push_back(map);
	}

	void removeVertexMap(Graph_vertex_map_base* map) {
  ...
  public:
	...
	Graph_2& operator=(const Graph_2& other) {
		...
		for (auto eit = other.m_edges.begin(); eit != other.m_edges.end(); ++eit) {
			...
			auto& e = m_edges.emplace_back(new_source, new_target, eit->m_curve, eit->m_data);
	...
	Graph_2 transform(CGAL::Aff_transformation_2<Kernel> trans) const {
		...
		for (auto eit = m_edges.begin(); eit != m_edges.end(); ++eit) {
			...
			transformed.m_edges.emplace_back(
			    new_source, new_target, Curve_traits::transform(eit->m_curve, trans), eit->m_data);
	...
	Vertex_handle add_vertex(Point_2 p) {
		...
		for (Graph_vertex_map_base* m : m_vertex_maps) {
			m->index_updated(-1, v.m_index);
		}
		return (--m_vertices.end());
	}
...
template <class G, typename T> class Graph_vertex_map : public Graph_vertex_map_base {
...
protected:
  ...
  public:
	Graph_vertex_map(G& graph, const T init) : m_init(init), m_graph(graph) {
		m_graph.addVertexMap(this);
		m_vec.resize(m_graph.number_of_vertices(), m_init);
	}

	~Graph_vertex_map() {
		m_graph.removeVertexMap(this);
	}
...
template <class G, typename T> class Graph_static_vertex_map {
  ...
  public:
	Graph_static_vertex_map(Graph& graph, const T init) {
...
template <class G> class Graph_vertex_index_map {
  public:
	...
	Graph_vertex_index_map(Graph& graph) {}
